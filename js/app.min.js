function addProductToCart(e,t){e.preventDefault();var r=$("div[data-product-id="+t+"]"),a=r.find(".product-img"),n=a.find("img");isCartFull()?$("#modalError").modal():(updateSummaryWithImage(n.attr("src"),getNextImageId()),cart.push({productId:t,src:n.attr("src")}),fireEventChanges())}function removeProductFromCart(e,t){e.preventDefault(),console.log("remove index "+t),cart.splice(t-1,1),rebuildSummaryItems(),fireEventChanges()}function rebuildSummaryItems(){for(i=1;7>i;i++)console.log("clearing item "+i),updateSummaryWithImage("",i);for(i=1;i<=cart.length;i++)console.log("adding item "+i),updateSummaryWithImage(cart[i-1].src,i)}function updateSummaryWithImage(e,t){var r=$(".item-"+t);r.addClass("selected");var a=r.find(".summary-item-img");""==e?(r.find(".summary-item-img").empty(),r.removeClass("selected")):a.html("<img onclick='removeProductFromCart(event,"+t+")' src='"+e+"'>")}function updateProgressIndicators(){areWeAtThree()?$(".summary-progress-3").addClass("complete"):$(".summary-progress-3").removeClass("complete"),areWeAtSix()?$(".summary-progress-6").addClass("complete"):$(".summary-progress-6").removeClass("complete")}function fireEventChanges(){updateProgressIndicators()}function getNextImageId(){return getNumberOfItemsInCart()+1}function isCartFull(){return cart.length>5}function isCartEmpty(){return cart.length-0}function getNumberOfItemsInCart(){return console.log("number of items in cart"+cart.length),cart.length}function areWeAtThree(){return cart.length>=3}function areWeAtSix(){return 6==cart.length}function bollox(){$(window).scrollTo($("progress-box"),300,{axis:"y"})}$(function(){function e(){return"ontouchstart"in window||"onmsgesturechange"in window}e()?$("html").addClass("touch"):$("html").addClass("non-touch"),$(window).load(function(){$("#loader").fadeOut(),$("#overlay").delay(800).fadeOut("slow")}),//!! start of app scripts !!//
